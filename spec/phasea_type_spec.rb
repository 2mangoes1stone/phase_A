...
describe 'PhaseAType' do
  ...
  describe 'phasea_phase_type' do # context is an alias for describe
    ...
    it "returns nil if invalid phase 3" do
      expect(phasea_phase_type([['2C', '2C', '4C', 'KC']])).to eq(nil)
    end
    ...
    it "returns the phase type 5.1" do
      expect(phasea_phase_type([['4H', '5D', 'AC', '7H']])).to eq(5.1)
    end

    it "returns nil if invalid phase 5.1" do
      expect(phasea_phase_type([['4H', '5D', '7H', 'AC']])).to eq(nil)
    end

    it "returns the phase type 5.2" do
      expect(phasea_phase_type([['2S', '3C', '4C', '5S']])).to eq(5.2)
    end

    it "returns the phase type 5.2" do
      expect(phasea_phase_type([['5D', '6H', '7D', '8H']])).to eq(5.2)
    end

    it "returns the phase type 5.2" do
      expect(phasea_phase_type([['5D', '6H', '7D', '8H', '9D', '0D', 'JH', 'QH', 'KD']])).to eq(5.2)
    end
